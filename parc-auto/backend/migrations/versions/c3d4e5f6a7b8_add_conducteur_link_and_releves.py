...
def upgrade():
    ...
    utilisateurs_columns = [c['name'] for c in inspector.get_columns('utilisateurs')]
    if 'conducteur_id' not in utilisateurs_columns:
        op.add_column(
            'utilisateurs',
    ...
    if 'releves_journaliers' not in tables:
        op.create_table(
            'releves_journaliers',
